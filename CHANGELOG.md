...
### Added
- New `:-` and `:+` macro modifiers, matching POSIX shell parameter expansion
  semantics: `${NAME:-default}` expands to `default` when `NAME` is empty,
  and `${NAME:+replace}` expands to `replace` when `NAME` is non-empty.
